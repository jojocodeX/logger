		...
		whereExpression.accept(new ExpressionVisitorAdapter() {
			...
			public void visit(Column expr) {
                if (!result.contains(expr.getColumnName())) {
                    if (expr.getColumnName().indexOf(".") == -1
                        && expr.getColumnName().toLowerCase().indexOf("id") > -1) {

                        result.add("id");

					} else {
                        result.add(MybatisUtils
                            .mapUnderscoreToCamelCase(expr.getColumnName().toLowerCase()));
                    }
                }
			}
